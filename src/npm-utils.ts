...
export async function getInstalledPackages(npmPath: string): Promise<string[]> {
  const globalPackages = await getPackageList(`${npmPath} list -g --json --depth=0 --no-unicode`)
  const localPackages = await getPackageList(`${npmPath} list --json --depth=0 --no-unicode`)
  const mergedPackages = [...globalPackages, ...localPackages]
  return mergedPackages
}

export async function importGlobalPackage(packageName: string, npmPath: string): Promise<any> {
  try {
    ...
    if ((error as any).code === 'MODULE_NOT_FOUND') {
      ...
      const { stdout: globalNodeModules } = await execAsync2(`${npmPath} root -g`)
